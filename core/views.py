...
def initialize(request):
    meta, created = MetaData.objects.get_or_create(
        defaults={"lastChecked": timezone.now(), "funds": 0}  # Set default values
    )

    current_month = timezone.now().month
    current_year = timezone.now().year

    # If lastChecked is None, set a default value
    lastChecked = str(meta.lastChecked) if meta.lastChecked else "00"
    lastChecked = lastChecked[5:7]  # Extract month part
    
    if lastChecked != str(current_month):
        # Member
        members = Member.objects.all()
        for member in members:
            if member.package.frequency == "monthly" and member.status:
